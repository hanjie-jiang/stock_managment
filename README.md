...
```powershell
schtasks /create /tn "StockTrackerBriefing" /tr "'C:\path\to\.venv\Scripts\python.exe' 'C:\path\to\jobs\run_daily_briefing.py'" /sc daily /st 07:00
```
...
app.py                    Streamlit entry point (streamlit run app.py) -- shared chrome
                           ...
                           pages/. Kept at root (not moved into shell/) since its
                           st.Page() calls resolve pages/ relative to its own location
shell/                    Streamlit presentation-layer helpers -- shared by app.py and
                           every page, but not a page itself
  ui_common.py               Styling, the pyarrow-free table renderer, sidebar
                             rendering, watchlist session-state helpers
  i18n.py                     English/Chinese static text + the structured reason-code
                             templates signals.py's bullish/bearish/risk/checklist
                             entries render through
  auto_shutdown.py            Exits the process once every browser tab has
                             disconnected, so the Desktop shortcut always starts fresh
pages/
...
stock_toolkit/            Core data + analysis package, reused by the app, the
                           notebook, and the background briefing job -- no Streamlit
...
data/                     Local JSON persistence
  ...
  settings_store.py           Per-install display language, in settings.json (gitignored)
...
launcher/                 Getting the app onto and running on a family member's PC --
                           audience is a person (or Windows), not the running app
...
jobs/                     Recurring background work invoked unattended (Task
                           Scheduler), not by a person clicking something
  run_daily_briefing.py      Pre-generates Today's Briefing for the whole watchlist
                             (see "Optional: Today's Briefing" above)
notebooks/
...
specs/                    Source of truth for new-feature intent -- 000-template.md,
                           then one numbered spec per nontrivial feature
tests/
